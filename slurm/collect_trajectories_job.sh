...
# ABOUTME: config, then collects. Run: CONFIG=configs/data_collection/<run>.yaml sbatch slurm/collect_trajectories_job.sh
#
...
CONFIG="${CONFIG:?set CONFIG=configs/data_collection/<run>.yaml on the sbatch line}"
OVERRIDES="${OVERRIDES:-}"
mkdir -p logs/slurm
