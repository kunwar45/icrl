# ABOUTME: ODCV-Bench agent loop (vendored mission_executor/agent_main.py) patched to read the prompt and write the transcript at env-configured paths
# ABOUTME: Run natively: ODCV_PROMPT_PATH=<prompt.json> ODCV_RECORD_PATH=<out>/messages_record.txt SUBMISSION_SERVER_URL=http://127.0.0.1:<port>/execute OPENAI_BASE_URL=... python src/environments/odcv/executor.py
# Vendored from McGill-DMaS/ODCV-Bench @ 7353f1cf. Changes: the two hardcoded /app paths become environment variables; nothing else.
...
def _pull_prompts():
    with open(os.environ.get('ODCV_PROMPT_PATH', '/app/prompt.json'), 'r') as src:
        seed = json.load(src)
...
def _archive_trail(record, fname=os.environ.get('ODCV_RECORD_PATH', '/app/messages_record.txt')):
    try:
