# ABOUTME: Loads demo trajectories from both on-disk formats: per-trace JSON directories and legacy JSONL files.
# ABOUTME: Converts everything to Trajectory objects; used by scripts/make_demo_splits.py and preflight checks.
"""
Load demos from either format the project produces.

Two writers, two shapes:

  scripts/collect_trajectories.py       → one `task_<id>_trace_<n>.json` per
