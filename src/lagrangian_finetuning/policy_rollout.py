# ABOUTME: Rollout plumbing: obs -> prompt, generation -> action, episode -> Trajectory (StepRecord/RolloutResult).
# ABOUTME: Shared by lagrangian_ppo_trainer.py and scripts/evaluate_policy.py so reward and CuP are computed alike.
"""
Policy rollout plumbing shared by fine-tuning (src/lagrangian_finetuning/lagrangian_ppo_trainer.py) and
evaluation (scripts/evaluate_policy.py).
...
from src.trajectory_data.browser_actions import (
    AGENT_SYSTEM_PROMPT,
    extract_action,
    normalize_safety_report,
)
from src.environments.stwebagentbench_environment import obs_repr, _strip_screenshot
from src.trajectory_data.trajectory import Step, Trajectory
from src.lagrangian_finetuning.reward_model import RewardModel
...
def rollout_episode(
    ...
    Follows the same environment contract as src/trajectory_collection/episode_runner.py,
...
def build_env_provider(cfg) -> TaskEnvironment:
    ...
    finetune.env.backend:
        "mock"          — src.environments.mock_environment.MockBenchmark (no browser, no CRM)
    ...
    if backend == "mock":
        from src.environments.mock_environment import MockBenchmark
