#!/usr/bin/env bash
# ABOUTME: Login-node session bootstrap: modules, venv, PYTHONPATH, SuiteCRM Apptainer instances, WA_SUITECRM
# ABOUTME: Source (do not execute) at the start of every SSH session: source scripts/start_session.sh
# Source this at the start of every SSH session on the login node.
#
# Usage:
#   source ~/projects/icrl/scripts/start_session.sh
#
# What it does:
...
export PYTHONPATH="${_ICRL_ROOT}/src/gridworld:${_ICRL_ROOT}/src:${PYTHONPATH:-}"
...
if [ -n "${_STORED_URL}" ] && curl -sf --max-time 5 "${_STORED_URL%/index.php}" > /dev/null 2>&1; then
    ...
    if [ -n "${_FOUND_URL}" ]; then
        ...
        if [ "$_HAS_MARIADB" -gt 0 ] && [ "$_HAS_SUITECRM" -gt 0 ]; then
            ...
            bash "${_ICRL_ROOT}/scripts/install_suitecrm_from_zip.sh" --restart
        else
            echo "      Installing SuiteCRM on $(hostname) (first time, ~2 min)..."
            bash "${_ICRL_ROOT}/scripts/install_suitecrm_from_zip.sh"
        fi
    fi
fi
...
echo ""
echo "To submit a job:  CONFIG=configs/trajectory_collection/stwebagentbench_expert.yaml sbatch scripts/slurm/collect_trajectories_job.sh"
