#!/usr/bin/env python3
# ABOUTME: Preflight checks (packages, GPUs, env vars, task registration, demos) before a job burns an allocation
# ABOUTME: Run: python scripts/run_preflight_checks.py [--backend stwebagent --task-ids ...]; exit 1 = not ready
"""
...
Usage:
    python scripts/run_preflight_checks.py                        # mock backend (no browser)
    python scripts/run_preflight_checks.py --backend stwebagent \
...
def check_repo_importable() -> None:
    try:
        import src.lagrangian_finetuning.policy_rollout  # noqa: F401
        ...
        if not code:
            ...
            if not defs:
                ...
                record(FAIL, "action set",
                       ...
                       "defined at module level (see src/environments/stwebagentbench_environment.py)")
            else:
