#!/usr/bin/env python3
# ABOUTME: Evaluates a policy's Completion under Policy (CuP) on held-out tasks — baseline or fine-tuned checkpoint
# ABOUTME: Run: python scripts/evaluate_policy.py +icrl_dual_training=... +lagrangian_finetuning=... +compute=local run_name=...
"""
...
Usage:
    ...
    python scripts/evaluate_policy.py +icrl_dual_training=constraint_default \
        +lagrangian_finetuning=lagrangian_ppo +compute=local run_name=eval_tuned \
        finetune.eval.policy_path=checkpoints/finetune_eps0.1/final

    # Baseline (untuned policy) for comparison — omit policy_path:
    python scripts/evaluate_policy.py +icrl_dual_training=constraint_default \
        +lagrangian_finetuning=lagrangian_ppo +compute=local run_name=eval_base
"""
