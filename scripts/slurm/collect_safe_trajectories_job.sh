#!/bin/bash
# ============================================================================
# collect_safe_trajectories_job.sh — Collect CuP=1 safe trajectories via vLLM + Qwen-72B
# ============================================================================
#
...
#   N_TASKS=3 sbatch scripts/slurm/collect_safe_trajectories_job.sh
#
#   # Explicit task IDs:
#   TASK_IDS="235 236 237 238 239" sbatch scripts/slurm/collect_safe_trajectories_job.sh
#
#   # More retries / longer episodes:
#   MAX_RETRIES=10 MAX_STEPS=40 sbatch scripts/slurm/collect_safe_trajectories_job.sh
#
#   # Dry-run (validate imports, env, vLLM health — no browser):
#   DRY_RUN=1 sbatch scripts/slurm/collect_safe_trajectories_job.sh
#
...
#                  on the login node (scripts/infra/start_suitecrm_apptainer.sh) and set
...
if [ -z "${WA_SUITECRM:-}" ]; then
    ...
    echo "  Start it from a login node: bash scripts/infra/install_suitecrm_direct.sh" >&2
    exit 1
fi
...
if [ "${DRY_RUN}" = "1" ]; then
    ...
    python scripts/demos/collect_safe_trajectories.py \
        --dry-run \
        --backend vllm \
        --vllm-url "${VLLM_URL}" \
        --model "${MODEL}" \
