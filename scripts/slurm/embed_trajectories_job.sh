#!/bin/bash
# ABOUTME: SLURM wrapper for scripts/embed_trajectories.py — pre-computes trajectory embeddings with frozen Qwen
# ABOUTME: Submit: sbatch scripts/slurm/embed_trajectories_job.sh; SAFE_DIR/UNSAFE_JSONL/BATCH/FLASH_ATTN env vars parameterize it
# ============================================================================
# embed_trajectories_job.sh — Pre-compute trajectory embeddings (frozen Qwen-1.5B)
# ============================================================================
#
...
#   sbatch scripts/slurm/embed_trajectories_job.sh
#
#   # Custom input / output paths:
#   SAFE_DIR=/scratch/$USER/traj/safe sbatch scripts/slurm/embed_trajectories_job.sh
#
#   # Larger batch (more VRAM) + flash attention (A100/H100):
#   BATCH=32 FLASH_ATTN=1 sbatch scripts/slurm/embed_trajectories_job.sh
#
#   # Dry-run: loads model and encodes one dummy text, then exits:
#   DRY_RUN=1 sbatch scripts/slurm/embed_trajectories_job.sh
#
...
cd "${ICRL_REPO}"
source "${ICRL_REPO}/scripts/slurm/job_environment.sh" 2>/dev/null || true
