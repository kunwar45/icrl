#!/bin/bash
# ABOUTME: SLURM wrapper for scripts/train_constraint.py — trains the ICRL constraint head on one GPU
# ABOUTME: Submit: sbatch scripts/slurm/train_constraint_job.sh [hydra overrides forwarded via "$@"]
