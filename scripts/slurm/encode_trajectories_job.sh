#!/bin/bash
# ============================================================================
# encode_trajectories_job.sh — Pre-compute trajectory embeddings (frozen Qwen-1.5B)
# ============================================================================
#
...
#
# Run this AFTER collect_safe_trajectories_job.sh has produced trajectory JSON files.
...
#   sbatch scripts/slurm/encode_trajectories_job.sh
#
#   # Custom input / output paths:
#   SAFE_DIR=/scratch/$USER/traj/safe sbatch scripts/slurm/encode_trajectories_job.sh
#
#   # Larger batch (more VRAM) + flash attention (A100/H100):
#   BATCH=32 FLASH_ATTN=1 sbatch scripts/slurm/encode_trajectories_job.sh
#
#   # Dry-run: loads model and encodes one dummy text, then exits:
#   DRY_RUN=1 sbatch scripts/slurm/encode_trajectories_job.sh
#
...
cd "${ICRL_REPO}"
source "${ICRL_REPO}/scripts/slurm/env.sh" 2>/dev/null || true
...
if [ -d "${SAFE_DIR}" ]; then
    ...
    python scripts/constraint/encode_trajectories.py \
        --input-dir "${SAFE_DIR}" \
        --label safe \
        --output "${SAFE_OUT}" \
        --model "${MODEL}" \
    ...
    echo "       Run collect_safe_trajectories_job.sh first."
fi
