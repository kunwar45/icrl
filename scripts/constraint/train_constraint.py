...
Produce those with `python scripts/demos/make_train_eval_splits.py`; the held-out split is what
scripts/constraint/eval_constraint.py scores, so the gate check stays honest.
...
Usage:
    python scripts/demos/make_train_eval_splits.py
    python scripts/constraint/train_constraint.py +constraint=icrl_default +compute=local \
    ...
    # Reuse embeddings from scripts/constraint/encode_trajectories.py (skips the backbone):
    python scripts/constraint/train_constraint.py +constraint=icrl_default +compute=local \
        run_name=constraint_v1 \
...
from src.constraint.encoder import TrajectoryEncoder, save_constraint_head
...
def _load_embeddings(path, expected_label: str):
    """Load an encode_trajectories.py bundle and return its (N, H) tensor."""
    if not path:
        return None
    if not os.path.exists(path):
        raise FileNotFoundError(
            f"Embedding bundle not found: {path}\n"
            f"Build it with: python scripts/constraint/encode_trajectories.py "
...
def main(cfg: DictConfig):
    ...
    for p in (expert_path, policy_path):
        if not os.path.exists(p):
            print(f"Missing {p}. Run: python scripts/demos/make_train_eval_splits.py", file=sys.stderr)
            sys.exit(1)
    ...
    # ── Train-set metrics (not the gate — see eval_constraint.py) ─────────────
    ...
    #   python scripts/constraint/eval_constraint.py ... run_name=<same run_name>
