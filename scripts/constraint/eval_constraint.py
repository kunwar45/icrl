...
Usage:
    python scripts/constraint/eval_constraint.py +constraint=icrl_default +compute=local \
...
def main(cfg: DictConfig):
    ...
    if not os.path.exists(head_path):
        print(f"No constraint head at {head_path}. Run scripts/constraint/train_constraint.py first.",
              file=sys.stderr)
        sys.exit(1)
