...
Usage:
    python scripts/sweep_finetune_hyperparams.py --mode local
    python scripts/sweep_finetune_hyperparams.py --mode slurm
"""
import subprocess
import itertools
import argparse
...
def main():
    ...
    for epsilon, seed in itertools.product(EPSILONS, SEEDS):
        ...
        if args.mode == "local":
            cmd = ["python", "scripts/finetune/run_finetune.py"] + cmd_args
            print(f"Running: {' '.join(cmd)}")
            subprocess.run(cmd, check=True)

        elif args.mode == "slurm":
            cmd = ["sbatch", "scripts/slurm/run_finetune_job.sh", "--", *cmd_args]
