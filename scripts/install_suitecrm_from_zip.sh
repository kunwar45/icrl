#!/usr/bin/env bash
# ABOUTME: One-time SuiteCRM 8 install via the Symfony console inside the Apptainer sandbox (bypasses Bitnami wizard)
# ABOUTME: Run once on the login node after seeding $SUITECRM_DATA/app: bash scripts/install_suitecrm_from_zip.sh [--restart]
...
#
# Usage:
#   bash scripts/install_suitecrm_from_zip.sh             # full install + start
#   bash scripts/install_suitecrm_from_zip.sh --restart   # skip install, just restart Apache
#
# Prerequisites:
...
if [ -z "${SUITECRM_HTTP_PORT:-}" ]; then
    ...
    PORT="${SUITECRM_HTTP_PORT}"
fi

# ── Restart-only mode: free port 8080, restart SuiteCRM instance ──────────────
if [ "${_RESTART_ONLY}" = "--restart" ]; then
    echo "Restart mode: freeing port ${PORT} and restarting SuiteCRM instance..."
    ...
    until curl -sf "http://localhost:${PORT}" >/dev/null 2>&1; do
        ...
        echo "  ${_waited}s..."
        [ "${_waited}" -ge 120 ] && { echo "Timeout" >&2; exit 1; }
    done
