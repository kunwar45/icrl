#!/usr/bin/env bash
# ABOUTME: Starts the SuiteCRM Docker compose stack (local dev) and waits until HTTP on localhost:8080 is healthy
# ABOUTME: Run before local trajectory collection: bash scripts/start_suitecrm_docker.sh
# Start SuiteCRM Docker container and wait for it to be healthy.
# Run this before any trajectory collection.
#
# Usage: bash scripts/start_suitecrm_docker.sh
set -e
