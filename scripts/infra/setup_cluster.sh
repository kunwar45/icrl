...
#   bash $ICRL_ROOT/scripts/infra/setup_cluster.sh
#
# Optional overrides:
...
# exporting SCRATCH=/project/<alloc>/$USER — same default as scripts/slurm/env.sh.
...
bash "${ICRL_ROOT}/scripts/infra/setup_fork_remotes.sh"
...
export PYTHONPATH="\${ICRL_ROOT}:\${ICRL_ROOT}/src:\${PYTHONPATH:-}"
EOF
...
export PYTHONPATH="${ICRL_ROOT}:${ICRL_ROOT}/src"
python -c "
...
log "  4. python ${ICRL_ROOT}/scripts/demos/smoke_test_demo_collection.py"
log "  5. For live browser on cluster: bash scripts/infra/start_suitecrm_apptainer.sh --wait, set WA_SUITECRM in .env"
