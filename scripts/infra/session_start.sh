...
#
# Usage:
#   source ~/projects/icrl/scripts/infra/session_start.sh
#
# What it does:
...
if [ ! -f "${_VENV}/bin/activate" ]; then
    ...
    echo "      Run setup first: bash ${_ICRL_ROOT}/scripts/infra/setup_cluster.sh"
    return 1
fi
...
export PYTHONPATH="${_ICRL_ROOT}:${_ICRL_ROOT}/src:${PYTHONPATH:-}"
...
if [ -n "${_STORED_URL}" ] && curl -sf --max-time 5 "${_STORED_URL%/index.php}" > /dev/null 2>&1; then
    ...
    if [ -n "${_FOUND_URL}" ]; then
        ...
        if [ "$_HAS_MARIADB" -gt 0 ] && [ "$_HAS_SUITECRM" -gt 0 ]; then
            ...
            bash "${_ICRL_ROOT}/scripts/infra/install_suitecrm_direct.sh" --restart
        else
            echo "      Installing SuiteCRM on $(hostname) (first time, ~2 min)..."
            bash "${_ICRL_ROOT}/scripts/infra/install_suitecrm_direct.sh"
        fi
    fi
fi
...
if [ -f "${_WA_ENV_FILE}" ]; then
    ...
    echo "      Run: bash ${_ICRL_ROOT}/scripts/infra/start_suitecrm_apptainer.sh"
fi
...
echo ""
echo "To submit a job:  sbatch ${_ICRL_ROOT}/scripts/slurm/collect_safe_trajectories_job.sh"
