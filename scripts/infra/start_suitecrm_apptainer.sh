...
#
# Usage:
#   bash scripts/infra/start_suitecrm_apptainer.sh                # start + wait for HTTP
#   bash scripts/infra/start_suitecrm_apptainer.sh --stop         # stop instances
#   bash scripts/infra/start_suitecrm_apptainer.sh --status       # list instances
#   bash scripts/infra/start_suitecrm_apptainer.sh --rebuild-sandbox  # re-extract SIF → sandbox
#   bash scripts/infra/start_suitecrm_apptainer.sh --fresh-install    # wipe data + reinstall
#
# Port notes:
...
case "${1:-}" in
    ...
    --rebuild-sandbox)
        ...
        build_sandbox
        echo "Done. Run 'bash scripts/infra/start_suitecrm_apptainer.sh' to start."
        ;;
    --fresh-install)
