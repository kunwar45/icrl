...
configs/data_collection/<run>.yaml         fully defines a run: benchmark, tasks,
...
slurm/collect_trajectories_job.sh          the ONLY way runs execute for real
```
...
DRY_RUN=1 sbatch --gres= --mem=4G --time=00:10:00 slurm/collect_trajectories_job.sh
```
...
`src/data/trace_loader.py` converts traces to the `Trajectory` dataclass, and
